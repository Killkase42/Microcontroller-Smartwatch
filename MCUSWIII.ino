...
//Defined constants for wifi module
...
int wifiSecs = 5; //Interval at which to scan for wifi networks
int numSsid = 0;
int lowBound = 0;  //The first index position in WiFi.SSID to be displayed; facilitates "scrolling" to see more networks
int upBound = 3; //The last index position in WiFi.SSID to be displayed
...
class Screens { //Class that covers the display of the watch, stopwatch, timer, sensor, and wifi module screens
  public:
    ...
    void modulesScreen() { //Display and function of the wifi module screen (currently inoperable)
      display.setTextSize(1);
      display.setCursor(5, 5);
      display.print("Wifi Module");
      //      if (wifiSecs == 5) { //Only scans for wifi networks once every 10 seconds
      //        numSsid = WiFi.scanNetworks();
      //        wifiSecs = 0; //Resets 10-second wifi scan time
      //      }
      display.setCursor(5, 20);
      if (WiFi.status() == WL_NO_MODULE) {
        display.print("Communication with WiFi module failed!");
      } else {
        display.print("Communication with WiFi module successful!");
      }
      //      display.print(numSsid);
      //      if (numSsid == -1) {
      //        display.setCursor(5, 15);
      //        display.print("No network connection");
      //      } else {
      //        if (numSsid < 4) { //If there are less than 4 networks, display only them
      //          upBound = numSsid;
      //        } else { //If there are 4 or more networks, display 4 within the selected range (allows scrolling because screen is too small to fit them all at once)
      //          upBound = lowBound + 4;
      //        }
      //
      //
      //        for (int thisNet = lowBound; thisNet < upBound; thisNet++) {
      //          display.setCursor(5, 15 + 10 * thisNet);
      //          display.print(thisNet + 1);
      //          display.print(") ");
...
void setup() {
  ...
  WiFi.setPins(SPIWIFI_SS, SPIWIFI_ACK, ESP32_RESETN, ESP32_GPIO0, &SPIWIFI); //Set up pins for wifi module
...
void increment_time() { //Increases watch and stopwatch hours/minutes/seconds, decreases for timer, over time
  ...
  if (currentMillis / 1000 - previousMillis / 1000 >= period / 1000) { //Check if 1000ms has passed; dividing by 1000 increases accuracy
    ...
    wifiSecs++; //Increment time for which wifi networks are scanned
    if (stopwatchOn) {
      stopwatch.second++;
    }
    if (timerOn) {
...
void loop() {
  ...
  switch (functionMode) { //Arrange attributes of each function mode on screen
    ...
    case 3: //Sensors
      ...
      button.func();
      break;
    case 4: //Wifi module, currently inoperable
